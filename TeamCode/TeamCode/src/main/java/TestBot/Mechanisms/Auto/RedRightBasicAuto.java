...
public class  RedRightBasicAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        encoderDrive(DRIVE_SPEED,  88,  -88, 5.0);  // strafe left Sec timeout
        encoderDrive(TURN_SPEED,   -8, -8, 4.0);  // forward
        encoderDrive(DRIVE_SPEED, 5,-5,2.0); //left again
        encoderDrive(DRIVE_SPEED,10,10,2); //go back
        runtime.reset();
        while(7.5>runtime.seconds()){           //SPIN DUCKY
            robot.duckyMover.setPower(-1.0);
        }
        robot.duckyMover.setPower(0);
        encoderDrive(DRIVE_SPEED,-7,7,4); //right
        encoderDrive(DRIVE_SPEED,4,4,2.0); //back
        turnLeft(TURN_SPEED,.9); //turn left to wall
        encoderDrive(DRIVE_SPEED,-6,6,2.0); // strafe 6 inches right


        encoderDrive(DRIVE_SPEED,-115,-115,5.0); //forward 78 inches to park
